...
equation Ai / Bi = values[i]. Each Ai or Bi is a string that represents 
a single variable.
...
values = [1.5,2.5,5.0], queries = [["a","c"],["c","b"],["bc","cd"],
["cd","bc"]]
...
# the inputs in equations act as a edge in graph
# add multiplications in heap so that you can get the desired div
# eg, push a/b * b/c to get a/c
# use a heap to push values until you reach the divident target
...
class Solution:
    def calcEquation(self, equations: List[List[str]], values: List[float], queries: List[List[str]]) -> List[float]:
        ...
        for i in range(len(equations)):
            gr.add_node(equations[i][0], equations[i][1], values[i])
        
        res = []
        
        for i in range(len(queries)):
            res.append(gr.find_div(queries[i][0], queries[i][1]))
